...
mapping each (network, station, year, yearday) to its S3 key and object size in bytes.
...
hitting S3 at download time, and by pair_relaxation_sweep.py to estimate download
volume for candidate distance/duration thresholds. `size_bytes` comes from the same
`list_objects_v2` response already being paginated for the key -- capturing it adds
zero additional S3 API calls.
            ...
            if 'Contents' in page:
                for obj in page['Contents']:
                    ...
                    if len(parts) >= 5:
                        ...
                        records.append((network, station, year, yearday, key, obj['Size']))
                ...
                for page in paginator.paginate(Bucket=bucket, Prefix=f"miniseed/{network}/"):
                    if 'Contents' in page:
                        for obj in page['Contents']:
                            ...
                            parts = key.split('/')
                            if len(parts) >= 5:
                                records.append((network, parts[-1].split('.')[0], parts[2], parts[3], key, obj['Size']))
...
def main():
    ...
    df = pd.DataFrame(all_records, columns=['network', 'station', 'year', 'yearday', 'dataacess_key', 'size_bytes'])
    ...
    raw_data_gb = df['size_bytes'].sum() / (1024 ** 3)
    ...
    print(f"  Total Raw Data:      {raw_data_gb:,.1f} GB (sum of all indexed object sizes)")
