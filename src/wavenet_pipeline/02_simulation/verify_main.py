...
def compute_ftan_and_plot(hdf5_path, output_png, target_model_key, frame_idx, total_frames):
    ...
    ccf_plot = time_ccf[lag_mask]
    ccf_max = np.max(np.abs(ccf_plot)) if len(ccf_plot) > 0 else 1.0
    if ccf_max > 0:
        ccf_plot = ccf_plot / ccf_max
    ax_ccf_time.plot(lags[lag_mask], ccf_plot, 'w-', lw=1)
    ax_ccf_time.set_ylim(-1.1, 1.1)
    ax_ccf_time.set_xlabel('Lag Time (s)', fontsize=9)
    ax_ccf_time.set_title(f'Time-Domain CCF (Normalized, Bandpass: {fmin}-{fmax} Hz)', fontsize=10)
