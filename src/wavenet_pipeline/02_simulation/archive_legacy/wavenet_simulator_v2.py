    ...
    try:
        ...
        with h5py.File(shard_path, 'a') as h5f:
            ...
            for _, geom in (geom_subset.head(1) if os.environ.get('TEST_1_GEOM') else geom_subset).iterrows():
                ...
                stack_fd_acc = {sc_name: {'Z1': np.zeros(len(freqs_rfft), dtype=np.complex128),
                                          'N1': np.zeros(len(freqs_rfft), dtype=np.complex128),
                                          'E1': np.zeros(len(freqs_rfft), dtype=np.complex128),
                                          'Z2': np.zeros(len(freqs_rfft), dtype=np.complex128),
                                          'N2': np.zeros(len(freqs_rfft), dtype=np.complex128),
                                          'E2': np.zeros(len(freqs_rfft), dtype=np.complex128)} 
                                for sc_name in ['ccf_isotropic', 'ccf_inline', 'ccf_broadside', 'ccf_onesided', 'ccf_random_50deg']}
                n_sources_acc = {sc_name: 0 for sc_name in stack_fd_acc.keys()}
                
                # Deterministic random state per geometry
                np.random.seed(42 + int(model_id.split('_')[-1]) + int(sep_km))
                
                for b in range(n_batches):
                    b_size = min(BATCH_SIZE, N_SOURCES - b*BATCH_SIZE)
                    
                    global_r = np.random.uniform(ANNULUS_RMIN, ANNULUS_RMAX, b_size)
                    global_theta = np.random.uniform(0, 360, b_size)
                    global_x = global_r * np.cos(np.deg2rad(global_theta))
                    global_y = global_r * np.sin(np.deg2rad(global_theta))
                    
                    global_fn = np.random.uniform(-1, 1, b_size)
                    global_fe = np.random.uniform(-1, 1, b_size)
                    global_fd = np.random.uniform(-1, 1, b_size)
                    
                    # Random time shifts inside TMAX
                    tshifts = np.random.uniform(0, TMAX, b_size)
                    phase_shifts = np.exp(-2j * np.pi * freqs_rfft[None, :] * tshifts[:, None])
                    
                    scenarios = {
                        'ccf_isotropic': np.ones(b_size, dtype=bool),
                    ...
                    # Pre-calculate FFTs for the entire batch to avoid redundant FFTs per scenario
                    ...
                    for sc_name, mask in scenarios.items():
                        ...
                        n_sources_acc[sc_name] += len(valid_idx)
                        ps_valid = phase_shifts[valid_idx]
                        
                        def accumulate_fd(waves_fd):
                            return np.sum(waves_fd[valid_idx] * ps_valid, axis=0)
                            
                        stack_fd_acc[sc_name]['Z1'] += accumulate_fd(Z1_fd)
                        stack_fd_acc[sc_name]['N1'] += accumulate_fd(N1_fd)
                        stack_fd_acc[sc_name]['E1'] += accumulate_fd(E1_fd)
                        stack_fd_acc[sc_name]['Z2'] += accumulate_fd(Z2_fd)
                        stack_fd_acc[sc_name]['N2'] += accumulate_fd(N2_fd)
                        stack_fd_acc[sc_name]['E2'] += accumulate_fd(E2_fd)
                        
                # After batch loop, compute inverse FFTs and CCFs
                for sc_name in stack_fd_acc.keys():
                    sc_grp = sim_grp.require_group(sc_name)
                    sc_grp.attrs['n_sources_used'] = int(n_sources_acc[sc_name])
                    
                    r1z = np.fft.irfft(stack_fd_acc[sc_name]['Z1'], n=nfft)[:stack_length]
                    r1n = np.fft.irfft(stack_fd_acc[sc_name]['N1'], n=nfft)[:stack_length]
                    r1e = np.fft.irfft(stack_fd_acc[sc_name]['E1'], n=nfft)[:stack_length]
                    r2z = np.fft.irfft(stack_fd_acc[sc_name]['Z2'], n=nfft)[:stack_length]
                    r2n = np.fft.irfft(stack_fd_acc[sc_name]['N2'], n=nfft)[:stack_length]
                    r2e = np.fft.irfft(stack_fd_acc[sc_name]['E2'], n=nfft)[:stack_length]
