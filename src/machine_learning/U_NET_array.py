...
from h5_wavenet_tools import HDF5Reader
...
# ---------------------------------------------------------------------------
# ---------------------------------------------------------------------------
# Dataset (Now uses HDF5Reader from h5_wavenet_tools)
# ---------------------------------------------------------------------------
    ...
    with torch.no_grad():
        for inputs, targets, metadata_batch in dataloader:
            ...
            for b in range(inputs.size(0)):
                ...
                domain    = get_val(metadata_batch['domain'])
                sim_id    = get_val(metadata_batch['simulation_id'])
                az_range  = get_val(metadata_batch['azimuth_range'])
                title_str = f"Sim {sim_id} | {domain} | az {az_range}"
...
def train_model(config):
    ...
    dataset    = HDF5Reader(config['hdf5_file'])
