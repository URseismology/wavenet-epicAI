#!/usr/bin/env python3
"""
Diagnostic plot for experiments 7-16 coverage analysis output folders.

Layout:
  [Source Distribution] [SREGN vs Extracted] [Full FTAN + curve]
  [Vp depth profile   ] [CCF time (top)     ] [                 ]
                        [Coherence (bottom)  ]

Usage:
    python plot_exp7_16.py <case_dir>
    python plot_exp7_16.py experiments/experiment_10/outputs/azimuthal_coverage_analysis
    python plot_exp7_16.py experiments/experiment_10
    python plot_exp7_16.py --all   # process experiments 7-16
"""

import sys
import re
import numpy as np
import matplotlib.pyplot as plt
import matplotlib.gridspec as gridspec
from pathlib import Path
import scipy.interpolate
from scipy.ndimage import gaussian_filter1d

try:
    import pycwt
except ImportError:
    print("Error: pycwt not installed.")
    sys.exit(1)

# ---------------------------------------------------------------------------
# Constants
# ---------------------------------------------------------------------------
MODEL_SUITE_DIR = Path('experiments/model_suite')
MOVIES_DIR      = Path('experiments/movies')

FTAN_FMIN = 0.05
FTAN_FMAX = 1.0
FTAN_VMIN = 0.5
FTAN_VMAX = 4.5
DT        = 0.5

FAMILY_COLORS = {
    'CIA':                    '#e41a1c',
    'CUS':                    '#377eb8',
    'Continental':            '#4daf4a',
    'Central_US_Continental': '#4daf4a',
    'Craton':                 '#984ea3',
    'Interior':               '#ff7f00',
    'KOREA':                  '#a65628',
    'Rift':                   '#f781bf',
    'Shield':                 '#999999',
    'WUS':                    '#ffff33',
    'tak135sph':              '#00ced1',
}


# ===========================================================================
# Readers
# ===========================================================================

def read_sregn(sregn_file):
    data = np.loadtxt(sregn_file, skiprows=1)
    mask = data[:, 0] == 0
    return data[mask, 2], data[mask, 5]


def read_mod_file(mod_file):
    with open(mod_file, 'r') as f:
        lines = f.readlines()
    data_start = None
    for i, line in enumerate(lines):
        if 'H(KM)' in line or 'H (KM)' in line:
            data_start = i + 1
            break
    if data_start is None:
        data_start = 0
    depths, vp_vals, vs_vals = [], [], []
    current_depth = 0.0
    for line in lines[data_start:]:
        parts = line.split()
        if len(parts) < 3:
            continue
        try:
            h  = float(parts[0])
            vp = float(parts[1])
            vs = float(parts[2])
        except ValueError:
            continue
        depths.append(current_depth); vp_vals.append(vp); vs_vals.append(vs)
        ext = current_depth + (h if h > 0 else 60.0)
        depths.append(ext);           vp_vals.append(vp); vs_vals.append(vs)
        if h > 0:
            current_depth += h
    return np.array(depths), np.array(vp_vals), np.array(vs_vals)


def compute_ftan(ccf_lags, ccf_amp, distance_km):
    npts = len(ccf_amp)
    indx = npts // 2
    data = 0.5 * ccf_amp[indx:] + 0.5 * np.flip(ccf_amp[:indx + 1], axis=0)

    pt1 = int(distance_km / FTAN_VMAX / DT)
    pt2 = int(distance_km / FTAN_VMIN / DT)
    if pt1 == 0: pt1 = 10
    if pt2 > (npts // 2): pt2 = npts // 2

    idx  = np.arange(pt1, pt2)
    tvec = idx * DT
    data = data[idx]

    cwt, _, freq, _, _, _ = pycwt.cwt(data, DT, 1/24, -1, -1, 'morlet')
    freq_ind = np.where((freq >= FTAN_FMIN) & (freq <= FTAN_FMAX))[0]
    cwt  = cwt[freq_ind]
    freq = freq[freq_ind]

    period = 1 / freq
    rcwt   = np.abs(cwt) ** 2

    per = np.arange(int(1 / FTAN_FMAX), int(1 / FTAN_FMIN), 0.25)
    vel = np.arange(FTAN_VMIN, FTAN_VMAX, 0.01)

    vel_data = distance_km / tvec
    fc       = scipy.interpolate.interp2d(vel_data, period, rcwt, kind='linear')
    rcwt_new = fc(vel, per)

    for ii in range(len(per)):
        mx = rcwt_new[ii].max()
        if mx > 0: rcwt_new[ii] /= mx

    for j in range(len(vel)):
        rcwt_new[:, j] = gaussian_filter1d(rcwt_new[:, j], sigma=0.15)

    return per, vel, rcwt_new


def extract_curve(per, vel, ftan_amp):
    nper, gv = [], []
    for ii in range(len(per)):
        idx = np.argmax(ftan_amp[ii])
        if ftan_amp[ii, idx] > 0.5:
            nper.append(per[ii])
            gv.append(vel[idx])
    return np.array(nper), np.array(gv)


def get_model_name_from_outputs(exp_outputs_dir):
    """Extract model name from sim folder names in outputs dir."""
    sim_dirs = list(exp_outputs_dir.glob('sim_*'))
    if not sim_dirs:
        return None
    # e.g. sim_00001_ang_0_2_dist_50_rad_400_600_WUS -> WUS
    m = re.search(r'_rad_\d+_\d+_(.+)$', sim_dirs[0].name)
    if m:
        return m.group(1)
    parts = sim_dirs[0].name.split('_')
    for part in reversed(parts):
        if part and not part.isdigit():
            return part
    return None


def get_dist_from_outputs(exp_outputs_dir):
    """Extract receiver distance from sim folder names."""
    sim_dirs = list(exp_outputs_dir.glob('sim_*'))
    if not sim_dirs:
        return 50
    m = re.search(r'_dist_(\d+)_', sim_dirs[0].name)
    return int(m.group(1)) if m else 50


# ===========================================================================
# Main plot
# ===========================================================================

def make_plot(case_dir, exp_outputs_dir, exp_num):
    case_dir = Path(case_dir)

    # ---- Parse case dir name -----------------------------------------------
    # e.g. case_01_full_az000-360
    m = re.match(r'case_(\d+)_(\w+)_az(\d+)-(\d+)', case_dir.name)
    if not m:
        print(f"  SKIP (cannot parse): {case_dir.name}")
        return
    case_id   = m.group(1)
    case_name = m.group(2)
    az_start  = int(m.group(3))
    az_end    = int(m.group(4))

    # ---- Locate required files ---------------------------------------------
    ccf_file    = case_dir / 'stacked_time_ccf.txt'
    sregn_file  = case_dir / 'SREGN.ASC'
    model_d     = case_dir / 'model.d'
    sources_csv = case_dir / 'all_sources.csv'
    coh_file    = case_dir / 'stacked_coherence_freq.txt'

    for f, name in [(ccf_file, 'stacked_time_ccf.txt'),
                    (sregn_file, 'SREGN.ASC'),
                    (model_d,    'model.d')]:
        if not f.exists():
            print(f"  SKIP {case_dir.name}: {name} not found")
            return

    # ---- Load data ---------------------------------------------------------
    ccf_data = np.loadtxt(ccf_file)
    ccf_lags = ccf_data[:, 0]
    ccf_amp  = ccf_data[:, 1]

    theory_per, theory_gvel = read_sregn(sregn_file)

    model_name = get_model_name_from_outputs(exp_outputs_dir)
    dist_km    = get_dist_from_outputs(exp_outputs_dir)

    print(f"  {case_dir.name}  |  model={model_name}  dist={dist_km}km")

    import pandas as pd
    sources_df = pd.read_csv(sources_csv) if sources_csv.exists() else None

    # ---- Compute FTAN ------------------------------------------------------
    per, vel, ftan_amp = compute_ftan(ccf_lags, ccf_amp, dist_km)
    per_picked, gv_picked = extract_curve(per, vel, ftan_amp)

    # ---- Load base models --------------------------------------------------
    base_models = {}
    if MODEL_SUITE_DIR.exists():
        for mf in sorted(MODEL_SUITE_DIR.glob('*.mod')):
            d, vp, vs = read_mod_file(mf)
            base_models[mf.stem] = (d, vp, vs)

    # ---- Figure layout -----------------------------------------------------
    fig = plt.figure(figsize=(20, 12), facecolor='#0d0d0d')
    gs  = gridspec.GridSpec(2, 3, figure=fig,
                            left=0.06, right=0.97,
                            top=0.90, bottom=0.07,
                            wspace=0.35, hspace=0.45)

    ax_src  = fig.add_subplot(gs[0, 0])
    ax_disp = fig.add_subplot(gs[0, 1])
    ax_ftan = fig.add_subplot(gs[0:, 2])
    ax_vp   = fig.add_subplot(gs[1, 0])

    gs_mid      = gridspec.GridSpecFromSubplotSpec(2, 1, subplot_spec=gs[1, 1], hspace=0.55)
    ax_ccf_time = fig.add_subplot(gs_mid[0])
    ax_coh      = fig.add_subplot(gs_mid[1])

    for ax in [ax_src, ax_disp, ax_ftan, ax_vp, ax_ccf_time, ax_coh]:
        ax.set_facecolor('#111111')
        ax.tick_params(colors='white', labelsize=8)
        for sp in ax.spines.values(): sp.set_edgecolor('#444444')
        ax.xaxis.label.set_color('white')
        ax.yaxis.label.set_color('white')
        ax.title.set_color('white')
        ax.grid(True, alpha=0.15, color='white')

    # ---- Source distribution -----------------------------------------------
    if sources_df is not None and len(sources_df) > 0:
        x, y = sources_df['x_km'], sources_df['y_km']
        n = len(x)
        if n > 10000:
            import random
            idx = random.sample(range(n), 10000)
            x, y = x.iloc[idx], y.iloc[idx]
        ax_src.scatter(x, y, s=1, c='white', alpha=0.3, edgecolors='none', rasterized=True)
        half = dist_km / 2
        ax_src.plot(-half, 0, 'rv', ms=8, mec='white', mew=0.5)
        ax_src.plot( half, 0, 'rv', ms=8, mec='white', mew=0.5)
        ax_src.plot([-half, half], [0, 0], 'w--', lw=1, alpha=0.6)
        ax_src.set_aspect('equal')
    ax_src.set_xlabel('X (km)', fontsize=9)
    ax_src.set_ylabel('Y (km)', fontsize=9)
    ax_src.set_title('Source Distribution', fontsize=10)

    # ---- Dispersion --------------------------------------------------------
    mask_t = theory_per <= 50
    ax_disp.plot(theory_per[mask_t], theory_gvel[mask_t], 'c-', lw=2, label='Theory (SREGN)')
    if len(per_picked) > 0:
        mask_e = per_picked <= 50
        ax_disp.plot(per_picked[mask_e], gv_picked[mask_e], '--', color='#ff9900', lw=2, label='Extracted')
        rms = np.sqrt(np.mean((gv_picked - np.interp(per_picked, theory_per, theory_gvel))**2))
        rel = rms / np.mean(np.interp(per_picked, theory_per, theory_gvel)) * 100
        ax_disp.set_title(f'Dispersion  RMS={rel:.1f}%', fontsize=10)
    else:
        ax_disp.set_title('Dispersion', fontsize=10)
    ax_disp.set_xlabel('Period (s)', fontsize=9)
    ax_disp.set_ylabel('Group Velocity (km/s)', fontsize=9)
    ax_disp.legend(fontsize=8, facecolor='#222222', labelcolor='white', framealpha=0.5)

    # ---- FTAN --------------------------------------------------------------
    ax_ftan.imshow(np.transpose(ftan_amp), cmap='inferno', aspect='auto',
                   origin='lower', extent=[per[0], per[-1], vel[0], vel[-1]],
                   vmin=0, vmax=1)
    mask_f = (theory_per >= per[0]) & (theory_per <= per[-1])
    ax_ftan.plot(theory_per[mask_f], theory_gvel[mask_f], 'c-', lw=2, label='Theory')
    if len(per_picked) > 0:
        ax_ftan.plot(per_picked, gv_picked, '--', color='#ff9900', lw=2, label='Extracted')
    ax_ftan.set_xlim(per[0], min(50, per[-1]))
    ax_ftan.set_xlabel('Period (s)', fontsize=9)
    ax_ftan.set_ylabel('Group Velocity (km/s)', fontsize=9)
    ax_ftan.set_title(f'FTAN  |  {case_name}  az{az_start}-{az_end}', fontsize=10)
    ax_ftan.legend(fontsize=8, facecolor='#222222', labelcolor='white', framealpha=0.5)

    # ---- Vp profile --------------------------------------------------------
    max_depth = 80.0
    for fam, (d, vp, vs) in base_models.items():
        mask = d <= max_depth
        is_current = (fam.lower() == (model_name or '').lower() or
                      fam.replace('_', '').lower() == (model_name or '').replace('_', '').lower())
        color  = FAMILY_COLORS.get(fam, '#888888') if is_current else '#888888'
        lw     = 2.5 if is_current else 0.8
        alpha  = 1.0 if is_current else 0.4
        zorder = 5   if is_current else 2
        label  = fam if is_current else None
        ax_vp.plot(vp[mask], d[mask], color=color, lw=lw, alpha=alpha,
                   zorder=zorder, label=label)
    ax_vp.set_xlabel('Vp (km/s)', fontsize=9)
    ax_vp.set_ylabel('Depth (km)', fontsize=9)
    ax_vp.invert_yaxis()
    ax_vp.set_ylim(max_depth, 0)
    ax_vp.set_title('P-wave Velocity', fontsize=10)
    ax_vp.legend(fontsize=8, facecolor='#222222', labelcolor='white',
                 framealpha=0.5, loc='lower right')

    # ---- CCF time domain ---------------------------------------------------
    LAG_MAX   = 100.0
    lags_mask = np.abs(ccf_lags) <= LAG_MAX
    ax_ccf_time.plot(ccf_lags[lags_mask], ccf_amp[lags_mask], 'w-', lw=0.8)
    ax_ccf_time.axvline(0, color='red', ls='--', lw=1, alpha=0.5)
    ax_ccf_time.axhline(0, color='gray', lw=0.5, alpha=0.3)
    ax_ccf_time.set_xlim(-LAG_MAX, LAG_MAX)
    ax_ccf_time.set_xlabel('Lag Time (s)', fontsize=9)
    ax_ccf_time.set_ylabel('Amplitude', fontsize=9)
    ax_ccf_time.set_title('Time-Domain CCF', fontsize=10)

    # ---- Coherence ---------------------------------------------------------
    if coh_file.exists():
        coh_data = np.loadtxt(coh_file)
        ax_coh.plot(coh_data[:, 0], coh_data[:, 1], color='#5599ff', lw=1)
        ax_coh.set_xlim(0, 0.5)
    ax_coh.set_xlabel('Frequency (Hz)', fontsize=9)
    ax_coh.set_ylabel('Coherence', fontsize=9)
    ax_coh.set_title('Frequency Coherence', fontsize=10)

    # ---- Super title -------------------------------------------------------
    fig.suptitle(f'Exp {exp_num}  |  {model_name}  |  Case {case_id}: {case_name}  |  '
                 f'dist={dist_km} km  |  az {az_start}-{az_end}°',
                 color='white', fontsize=13, fontweight='bold', y=0.96)

    # ---- Save --------------------------------------------------------------
    MOVIES_DIR.mkdir(parents=True, exist_ok=True)
    movie_name = f"exp{exp_num}_{model_name}_dist{dist_km}_{case_name}_az{az_start}-{az_end}.png"

    out_case  = case_dir / 'diagnostic_plot.png'
    out_movie = MOVIES_DIR / movie_name

    plt.savefig(out_case,  dpi=150, bbox_inches='tight', facecolor=fig.get_facecolor())
    plt.savefig(out_movie, dpi=150, bbox_inches='tight', facecolor=fig.get_facecolor())
    plt.close()
    print(f"    Saved: {out_movie.name}")


def process_analysis_dir(analysis_dir, exp_outputs_dir, exp_num):
    """Process all case dirs in an azimuthal_coverage_analysis folder."""
    case_dirs = sorted(analysis_dir.glob('case_*'))
    print(f"  Found {len(case_dirs)} cases in exp {exp_num}")
    for cd in case_dirs:
        if not cd.is_dir(): continue
        if (cd / 'diagnostic_plot.png').exists():
            print(f"  SKIP (done): {cd.name}")
            continue
        try:
            make_plot(cd, exp_outputs_dir, exp_num)
        except Exception as e:
            print(f"  ERROR {cd.name}: {e}")


# ===========================================================================
# Entry point
# ===========================================================================

if __name__ == '__main__':
    if len(sys.argv) < 2:
        print("Usage: python plot_exp7_16.py <case_dir|analysis_dir|exp_dir|--all>")
        sys.exit(1)

    arg = sys.argv[1]

    if arg == '--all':
        for exp_num in range(7, 17):
            exp_outputs = Path(f'experiments/experiment_{exp_num}/outputs')
            analysis    = exp_outputs / 'azimuthal_coverage_analysis'
            if not analysis.exists():
                print(f"Exp {exp_num}: no azimuthal_coverage_analysis dir, skipping")
                continue
            print(f"\n{'='*60}")
            print(f"Experiment {exp_num}")
            print('='*60)
            process_analysis_dir(analysis, exp_outputs, exp_num)
    else:
        p = Path(arg)
        if not p.exists():
            print(f"ERROR: {p} not found")
            sys.exit(1)

        # Single case dir
        if re.match(r'case_\d+_', p.name):
            exp_outputs = p.parents[2]
            m = re.search(r'experiment_(\d+)', str(p))
            exp_num = int(m.group(1)) if m else 0
            make_plot(p, exp_outputs, exp_num)

        # analysis dir or experiment dir
        elif p.is_dir():
            analysis = p / 'azimuthal_coverage_analysis' if not p.name.startswith('azimuthal') else p
            exp_outputs = analysis.parent if analysis.name == 'azimuthal_coverage_analysis' else p
            m = re.search(r'experiment_(\d+)', str(p))
            exp_num = int(m.group(1)) if m else 0
            process_analysis_dir(analysis, exp_outputs, exp_num)