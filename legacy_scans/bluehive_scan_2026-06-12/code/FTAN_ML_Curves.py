#!/usr/bin/env python3
"""
Generate FTAN heatmaps WITH dispersion curve overlays for ML training

Version B: Input with curve → Target with curve
- Observed: FTAN heatmap + extracted dispersion curve (cyan)
- Theoretical: FTAN heatmap + theoretical dispersion curve (cyan)
- Both 256x256 RGB PNG format
"""
...
class FTAN_ML_WithCurves:
    ...
    def extract_dispersion(self):
        """Extract dispersion curve from FTAN"""
        nper = []
        gv = []
    ...
    def save_ml_image_with_curve(self, output_dir, data_type='observed', img_size=256):
        """Save 256x256 RGB PNG with dispersion curve overlay (CYAN)"""
        ...
        # Overlay dispersion curve in CYAN
        if data_type == 'observed':
            # Use extracted dispersion
            if len(self.periods_picked) > 0:
                ax.plot(self.periods_picked, self.group_velocities,
                       'c-', linewidth=2.5, alpha=0.95)  # Cyan solid line
        else:
            # Use theoretical dispersion
            ...
            ax.plot(self.theory_periods[theory_mask], self.theory_gvel[theory_mask],
                   'c-', linewidth=2.5, alpha=0.95)  # Cyan solid line
...
def create_theoretical_ftan_map(periods, group_velocities,
                                period_range=(1, 20), velocity_range=(0.5, 4.5),
                                period_spacing=0.25, velocity_spacing=0.01,
                                line_width=5):
    """Create theoretical FTAN dispersion map (same as before)"""
...
def generate_theoretical_ftan_with_curve(sregn_file, model_name, az_range, rad_range, 
                                         distance_km, exp_num, output_dir, img_size=256):
    """Generate theoretical FTAN WITH theoretical curve overlay (CYAN)"""
    data = np.loadtxt(sregn_file, skiprows=1)
    mode0_mask = data[:, 0] == 0
    ...
    # Create heatmap
    period_grid, velocity_grid, amplitude_map = create_theoretical_ftan_map(
    ...
    # Overlay theoretical curve in CYAN
    theory_mask = (theory_periods >= period_grid[0]) & (theory_periods <= period_grid[-1])
    theory_mask &= (theory_gvel >= velocity_grid[0]) & (theory_gvel <= velocity_grid[-1])
    ax.plot(theory_periods[theory_mask], theory_gvel[theory_mask],
           'c-', linewidth=2.5, alpha=0.95)  # Cyan solid line
...
def process_experiment(exp_num, output_dir, theoretical_dir, metadata_list, img_size=256):
    ...
    for case_dir in case_dirs:
        ...
        try:
            # Generate OBSERVED FTAN with extracted curve
            ftan = FTAN_ML_WithCurves(ccf_file, distance_km, sregn_file, model_name, 
                                     az_range, rad_range, exp_num)
            ftan.compute_ftan(fmin=0.05, fmax=1.0, vmin=0.5, vmax=4.5)
            ftan.extract_dispersion()
            
            # Save observed with cyan curve overlay
            filename = ftan.save_ml_image_with_curve(output_dir, data_type='observed', img_size=img_size)
            metadata_list.append(ftan.get_metadata(filename, data_type='observed'))
            successful += 1
            
            # Generate THEORETICAL FTAN with theoretical curve
            theo_filename, theo_metadata = generate_theoretical_ftan_with_curve(
                sregn_file, model_name, az_range, rad_range, distance_km, exp_num, 
                theoretical_dir, img_size=img_size
            )
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python generate_ml_ftan_with_curves.py <exp1> [exp2] [exp3] ...")
        print("Example: python generate_ml_ftan_with_curves.py 6 7 8")
        sys.exit(1)
    ...
    print("="*70)
    print("FTAN ML INPUT GENERATION - WITH DISPERSION CURVES")
    print("VERSION B: Input with curve → Target with curve")
    print("="*70)
    print(f"Experiments: {experiment_numbers}")
    print(f"Output: FTAN_ML_INPUT_WITH_CURVES/ (observed + extracted curve)")
    print(f"        FTAN_ML_INPUT_WITH_CURVES/theoretical/ (theoretical + theory curve)")
    print(f"Format: {img_size}x{img_size} RGB PNG, inferno colormap, CYAN curves")
    print(f"Ranges: Period 1-20s, Velocity 0.5-4.5 km/s")
    print(f"Curve: Cyan solid line, linewidth=2.5")
    
    output_dir = Path("FTAN_ML_INPUT_WITH_CURVES")
    ...
    for exp_num in experiment_numbers:
        obs, theo, failed = process_experiment(exp_num, output_dir, theoretical_dir, 
                                               metadata_list, img_size=img_size)
        total_observed += obs
    ...
    if metadata_list:
        ...
        print("SUMMARY")
        print('='*70)
        print(f"Observed FTANs (with extracted curve): {total_observed}")
        print(f"Theoretical FTANs (with theory curve): {total_theoretical}")
