...
JOB_MAP="${ROOT}/job_map_roundrobin.csv"
OUTPUT_BASE="${ROOT}/outputs"

BATCH_SIZE=$1
numSubJobs=$2
OFFSET=$(( (SLURM_ARRAY_TASK_ID-1) * numSubJobs +1))

if (( $# < 2 )) 
then
  echo "submit_BH3_combined.sh Error: two arguments needed!!"
  exit 1
fi
...
# 1. Disable the Multi-Level (ML) hierarchy discovery that is crashing
export OMPI_MCA_coll_ml_enable=0

# 2. Tell UCX exactly which transports to use 
# 'rc' is Reliable Connected (InfiniBand), 'self' is loopback, 'shm' is shared memory
export UCX_TLS=rc,self,shm

# 3. Tell OpenMPI to use UCX for all point-to-point communication
...
#  echo "Array Task ID  : ${SLURM_ARRAY_TASK_ID}"
  ...
  echo "Starting MPI simulation index ${i_subJob} on ${SLURM_NTASKS}..."
  
  tt0=$SECONDS
  
  #mpirun  -n $SLURM_NTASKS python3 ${ROOT}/worker_point_forces_bl.py \
  #mpiexec -n $SLURM_NTASKS python3 ${ROOT}/worker_point_forces_bl.py \
  #srun --export=ALL python3 ${ROOT}/worker_point_forces_bl.py \
  ## runs >2 times slower with --oversubscribe
  ##mpirun --oversubscribe -n $SLURM_NTASKS python3 ${ROOT}/worker_point_forces_bl.py "$JOB_MAP" "$OUTPUT_DIR" "$ROW"
