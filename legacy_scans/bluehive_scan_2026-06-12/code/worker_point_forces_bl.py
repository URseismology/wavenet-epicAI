...
def precompute_greens_functions(model_file, workdir, r_max, rank):
    ...
    cmds = [
        ...
        'sregn96 -NOQ',
        'slegn96 -NOQ',
        'sdpegn96 -R -U -ASC',  # Creates SDISPR.ASC with C and U
        'sdpegn96 -L -U -ASC',  # Creates SDISPL.ASC with C and U
        'sdpsrf96 -R -ASC',     #  Rayleigh ASCII from dispersion
        'sdpsrf96 -L -ASC'      #  Love ASCII from dispersion
...
## B.Liu
##Vectorized Force Rotation & Projection
def compute_mt_to_zne_vectorized(greens, fR, fT, fZ, baz_rad):
    # baz_rad is backazimuth in radians
    uZ = fZ * greens['ZVF'] + fR * greens['ZHF']
    uR = fR * greens['RHF'] + fZ * greens['RVF']
    uT = fT * greens.get('THF', 0.0)

    uN = -np.cos(baz_rad) * uR + np.sin(baz_rad) * uT
    uE = -np.sin(baz_rad) * uR - np.cos(baz_rad) * uT
    return uZ, uN, uE
...
def run_simulation(config_file, model_file, output_dir, sim_id):
    ...
    if rank == 0:
        ...
        eigen_files = ['model.d', 'sregn96.egn', 'slegn96.egn',
               ...
               'SDISPR.ASC', 'SDISPL.ASC', 'SRAYR.ASC', 'SLOVE.ASC']
