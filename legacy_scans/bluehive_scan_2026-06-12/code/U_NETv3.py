#!/usr/bin/env python3
"""
Train UNet for FTAN Dispersion Curve Segmentation - WITH SHARPENING ENABLED

Features:
- Binary target masks (threshold 0.1)
- Focal Loss + Dice Loss + Weighted BCE + FIXED Sharpening Loss
- Sharpening forces confident predictions (close to 0 or 1)
- Metadata display right above each row of images
"""
...
# ============================================================================
# Loss Functions - WITH FIXED SHARPENING
# ============================================================================
...
class SharpeningLoss(nn.Module):
    """
    FIXED: Penalize uncertain predictions on curve pixels
    Forces model to output confident values (close to 0 or 1)
    """
    def __init__(self):
    ...
    def forward(self, pred, target):
        ...
        # Clamp to prevent log(0) errors
        pred_prob = torch.clamp(pred_prob, 1e-7, 1.0 - 1e-7)
        
        # Entropy: high for uncertain predictions (near 0.5), low for confident (near 0 or 1)
        entropy = -pred_prob * torch.log(pred_prob) - (1.0 - pred_prob) * torch.log(1.0 - pred_prob)
        
        # Only apply to curve pixels
        curve_mask = (target > 0.5).float()
        
        # Safe computation - use masked mean
        n_curve = curve_mask.sum()
        
        if n_curve > 0:
            # Weight entropy by curve mask and normalize
            sharpening_loss = (entropy * curve_mask).sum() / n_curve
        else:
            # No curve pixels - return zero loss on same device as input
...
# ============================================================================
# Visualization WITH METADATA - TEXT RIGHT ABOVE IMAGES
# ============================================================================

def save_validation_samples(model, dataloader, epoch, save_dir, device, num_samples=2):
    """Save validation samples WITH METADATA RIGHT ABOVE EACH ROW"""
    model.eval()
    ...
    with torch.no_grad():
        for i, (inputs, targets, metadata_batch) in enumerate(dataloader):
            ...
            # Add text RIGHT ABOVE each row of images
...
def train_model(config):
    ...
    # Loss and optimizer - SHARPENING ENABLED
...
def main():
    ...
    print("WITH SHARPENING LOSS ENABLED")
    print("="*70)
    print("\nConfiguration:")
    ...
    print(f"  Loss: Focal + Dice + Weighted BCE + Sharpening (ENABLED)")
    print(f"  Metadata display: Right above each row of images")
    print()
