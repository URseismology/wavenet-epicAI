#!/usr/bin/env python3
"""
Train UNet for FTAN Dispersion Curve Segmentation - WITH METADATA DISPLAY
...
class SharpeningLoss(nn.Module):
    ...
    def forward(self, pred, target):
        pred_prob = torch.sigmoid(pred)
        entropy = -pred_prob * torch.log(pred_prob + 1e-7) - (1 - pred_prob) * torch.log(1 - pred_prob + 1e-7)
        curve_mask = target > 0.5
        
        n_curve_pixels = curve_mask.sum().item()
        if n_curve_pixels > 0:
            sharpening_loss = (entropy * curve_mask.float()).sum() / curve_mask.sum()
        else:
            sharpening_loss = torch.tensor(0.0, device=pred.device)
        
        return sharpening_loss


class CombinedLoss(nn.Module):
    def __init__(self, focal_weight=0.3, dice_weight=0.3, bce_weight=0.2, sharpen_weight=0.0):
...
def save_validation_samples(model, dataloader, epoch, save_dir, device, num_samples=2):
    ...
    with torch.no_grad():
        for i, (inputs, targets, metadata_batch) in enumerate(dataloader):
            ...
            exp_num = metadata['experiment']
            az_start = int(metadata['azimuth_start'])
            az_end = int(metadata['azimuth_end'])
            distance = int(float(metadata['distance_km']))
            rad_min = int(float(metadata['radius_min']))
            rad_max = int(float(metadata['radius_max']))
...
def train_model(config):
    ...
    # Loss and optimizer
    criterion = CombinedLoss(focal_weight=0.3, dice_weight=0.3, bce_weight=0.2, sharpen_weight=0.0)
...
def main():
    ...
    print(f"  Loss: Focal + Dice + Weighted BCE + Sharpening")
