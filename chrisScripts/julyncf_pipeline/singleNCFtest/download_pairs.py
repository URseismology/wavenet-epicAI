...
Script for downloading the raw Miniseed files into a temporary directory

Characterisitcs: 

- Authentication lock so that each thread can change variables without issue 
- Progress tracking and statistics  
- Parquet index loading  
- Filtering by networks and number of pairs when given the arguments 

Example: 

python download_pairs.py \
  --pairs "/path/to/filtered_pairs.csv" \
  --outdir "/path/to/output/downloaded_pairs" \
  --keyindex "/path/to/index.parquet" \
  --networks "CN,US" \
  --max-pairs 500 \
  --workers 8
...
AUTH_LOCK = threading.Lock() # Lock so we have thread safe operations

# Initialization of global parameters
global_s3_client = None
global_es_client = None
CUSTOM_REFRESH_CALLBACK = None
...
def load_and_filter_pairs(pairs_file, networks=None, max_pairs=None):
    """ Loads the pairs based on what networks and max_pairs is chosen"""
...
def build_availability_from_parquet(keyindex_path, unique_stations):
    """ Makes a dictionary of station availability based on the parquet file given"""
    ...
    # reads the parquet file and filters for the networks and stations we want
    df = pd.read_parquet(
        keyindex_path,
        filters=[
            ('network', 'in', nets),
    ...
    # Check for station availability
...
def download_worker(item):
    """ Download worker function for downloading using multiple threads"""
    # Refrencing to global variables so that we use those
    ...
    # Checks to see if the file already exists and skips if it does
    ...
    try:
        # Gets the file object, reads the Body, and writes it to the local path
    ...
    # Handles AWS Token Expiration
        ...
        if "ExpiredToken" in error_str or "Token expired" in error_str or "Forbidden" in error_str:
            with AUTH_LOCK:
                ...
                if CUSTOM_REFRESH_CALLBACK:
                    global_s3_client = CUSTOM_REFRESH_CALLBACK()
                else:
...
def main():
    ...
    parser.add_argument("--networks", nargs="+", default=None, help="Filter by network codes")
    parser.add_argument("--workers", type=int, default=50, help="Number of parallel download threads")
    parser.add_argument("--max-days", type=int, default=None,
                        help="Maximum overlapping days to download per pair (default: all)")
    ...
    # Authentication with EarthScope
    ...
    # Loads and filters the pairs from the CSV file
    ...
    # Gets all unique stations from the pairs
    stations = set()
    ...
    for _, row in df_pairs.iterrows():
        ...
        if overlap_dates:
            ...
            # Adds all files for overlapping days to the download set
    ...
    # Initializes variables for tracking progress
    ...
    # Sorts the download set for consistency and gives threads what they need
    to_download_sorted = sorted(list(to_download))
    download_tasks = [(BUCKET, args.outdir, sta, key) for sta, key in to_download_sorted]
    
    # Downloads files in parallel using multithreading
    ...
    print("\n")
    # Saves all download information into a csv file 
